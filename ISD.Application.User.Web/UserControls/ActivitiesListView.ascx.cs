...
namespace ISD.User.Web.UserControls
{
    ...
    {
        public delegate void SectionHandler(int ID, int startRow, string sortValue, int pageSize, string searchKey, DateTime dtFrom, DateTime dtTo, int ageFrom, int ageTo, string SuburbID, bool monFilter, bool tueFilter, bool wedFilter, bool thuFilter, bool friFilter, bool satFilter, bool sunFilter, bool filtered);
        ...
        protected void ddlPagingTop_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                RefreshActivitiesSection(ID, 0, ddSort.SelectedValue, PageSize, SearchKey, dtFrom, dtTo, AgeFrom, AgeTo, SuburbID, MonFilter, TueFilter, WedFilter, ThursFilter, FriFilter, SatFilter, SunFilter, Filtered);
            }
        }
            ...
            if (!e.CommandName.Equals(string.Empty))
            {
                if (e.CommandName == "ToggleSave")
                {
                    ...
                    if (Convert.ToBoolean(hdnisSaved.Value))
                    {
                        ...
                        new DataAccessComponent().RemoveFromSavedList(IdentityHelper.GetProviderNameFromRequest(Request), userID, Convert.ToInt32(hdnActivityID.Value));
                    ...
                    else
                    {
                        ...
                        dr.OwnerGuid = IdentityHelper.GetUserIdFromRequest(Request);
                        dr.CreatedBy = IdentityHelper.GetProviderNameFromRequest(Request);;
        ...
        protected void ListViewActivities_PagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
        {
            ...
            if (RefreshActivitiesSection != null)
            {
                RefreshActivitiesSection(ID, startRow, ddSort.SelectedValue, PageSize, SearchKey, dtFrom, dtTo, AgeFrom, AgeTo, SuburbID, MonFilter, TueFilter, WedFilter, ThursFilter, FriFilter, SatFilter, SunFilter, Filtered);
            }
        }
        ...
        protected void ListViewActivities_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            ...
            actName = actName.Replace(":", string.Empty);
            if (actName.EndsWith("."))
                actName.TrimEnd('.');
            ...
            else
            {
                ...
                if (new DataAccessComponent().IsUserImageExist(hdnProviderID.Value))
                {
                    int ImageID = new BusinessFunctionComponent().getProviderPrimaryImage(hdnProviderID.Value);
                    if (ImageID != 0)
