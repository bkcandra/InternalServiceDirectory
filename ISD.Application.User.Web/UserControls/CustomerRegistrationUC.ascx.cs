...
namespace ISD.User.Web.UserControls
{
    public partial class CustomerRegistrationUC1 : System.Web.UI.UserControl
    {
        ...
        protected void validateUser(out bool error)
        {
            ...
            var user = manager.FindByNameAsync(Username.Text);
            if (user != null)
            {
                if (!string.IsNullOrEmpty(ErrorText))
                    ErrorText += "</br>";
        ...
        protected void StepNextButton_Click(object sender, EventArgs e)
        {
            ...
            if (!error)
            {
                ...
                var manager = Context.GetOwinContext().GetUserManager<ApplicationUserManager>();
                var newUser = new ApplicationUser() { UserName = Email.Text, Email = Email.Text };
                string token = manager.GenerateEmailConfirmationToken(newUser.Id);
                string callbackUrl = IdentityHelper.GetUserConfirmationRedirectUrl(token, newUser.Id);


                var RoleManager = new RoleManager<IdentityRole>(new RoleStore<IdentityRole>(new ISDEntities()));
                if (!RoleManager.RoleExists(SystemConstants.CustomerRole))
                {
                    var roleresult = RoleManager.Create(new IdentityRole(SystemConstants.CustomerRole));
                    var um = new UserManager<ApplicationUser>(
                    new UserStore<ApplicationUser>(new ApplicationDbContext()));
                    um.AddToRoleAsync(newUser.Id, SystemConstants.CustomerRole);
                ...
                SendConfirmationEmail(newUser.Id, callbackUrl);
        ...
        private void SendConfirmationEmail(String userID, string ConfirmationUrl)
        {
            ...
            new BusinessFunctionComponent().ParseEmail(emTemp, userID, ConfirmationUrl, (int)SystemConstants.EmailTemplateType.ConfirmationEmail, 0);
