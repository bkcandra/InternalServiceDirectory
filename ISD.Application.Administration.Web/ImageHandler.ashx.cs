    ...
    public class ImageHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (context.Request.QueryString[SystemConstants.qs_ImageID] != null)
            {
                ...
                if (imgID != 0)
                {
                    byte[] imageByte = new DataAccessComponent().RetrieveImageBinary(imgID);
            ...
            if (imgID != 0)
            {
                ...
                byte[] imageStream = new DataAccessComponent().RetrieveAssetBinary(imgID);
                if (imageStream != null)
                {
            ...
            {
                // Retrieve ImageStream from db
                byte[] imageStream = new DataAccessComponent().RetrieveUserImageBinary(UserimgID);
                if (imageStream != null)
                {
                    ImageResponse(context, imageStream);
                    context.Response.End();
            ...
            if (RewardID != 0)
            { // Retrieve ImageStream from db
                byte[] imageStream = new DataAccessComponent().RetrieveRewardBinary(RewardID);
                if (imageStream != null)
                {
                    ImageResponse(context, imageStream);
                    context.Response.End();
