...
namespace HealthyClub.Administration.Web.UserControls
{
    public partial class PageSetupUC : System.Web.UI.UserControl
    {
        ...
        protected void lnkCopyFrom_Click(object sender, EventArgs e)
        {
            ...
            new BusinessFunctionComponent().DuplicatePage(oldDynamicPageID, out newDynamicPageID);
            Response.Redirect("~/Pages/PageSetup.aspx?" + SystemConstants.PageID + "=" + newDynamicPageID + "&" + SystemConstants.EditMode + "=" + true);
        }

        private void SetData(DataSetComponent.PageRow dr)
        {
        ...
        protected void lnkSave_Click(object sender, EventArgs e)
        {
            ...
            var dr = GetData();
                ...
                if (Mode == UIMode.Create)
                {
                    if (Validate(dr))
                    {
                    new DataAccessComponent().CreatePage(dr);
                    PageID = dr.ID;
                    }
                }
                else if (Mode == UIMode.Edit)
                    new DataAccessComponent().UpdatePage(dr);
                Refresh();
            
        }

        private bool Validate(DataSetComponent.PageRow dr)
        {
            bool isvalid = true;
        ...
        {
            if (new DataAccessComponent().isPageExist(name))
                return false;
            else return true;
        ...
        private DataSetComponent.PageRow GetData()
        {
            DataSetComponent.PageRow dr = new DataSetComponent.PageDataTable().NewPageRow();
        ...
        {
            var dt = new DataAccessComponent().RetrievePages((int)SystemConstants.ItemType.Standard);
        ...
        {
            int newPageID = 0;
            new BusinessFunctionComponent().DuplicatePage(PageID, out newPageID);
