...
namespace HealthyClub.Providers.Web.UserControls
{
    ...
    
    {
        ...
        {
            DataList1.DataSource = new DataAccessComponent().RetrieveActivityImages(ActivityID);
        }

        private void SetImage(DataSetComponent.ActivityContactDetailRow dr)
        {
        ...
        private void CheckActivityImage()
        {
            ...
            if (ActivityID != 0)
            {
                var dr = new DataAccessComponent().RetrieveActivityImageInformation(ActivityID);
                if (dr == null)
                {
                    dr = new DataSetComponent.ActivityImageDataTable().NewActivityImageRow();
                    ...
                    new DataAccessComponent().createActivityImageInformation(dr);
        ...
        private DataSetComponent.ActivityImageDetailRow GetData()
        {
            int imagesCount = new DataAccessComponent().RetrieveActivityImagesCount(ActivityID);
        ...
        {
            ImageUploaded = 0;
            var drInfo = new DataAccessComponent().RetrieveActivityImageInformation(ActivityID);
            ...
            if (fileUpload1.HasFile)     // CHECK IF ANY FILE HAS BEEN SELECTED.
            {
                ...
                if (System.Text.RegularExpressions.Regex.IsMatch(fileUpload1.PostedFile.ContentType, "image/\\S+"))
                {
                    ...
                    if (ImageUploaded > 0)
                    {
                        ...
                        new BusinessFunctionComponent().CreateActivityImages(ActivityID, drInfo, dt);
                    }
         ...
         if (fileUpload1.HasFile)
            {
                ...
                lblUploadStatus.Visible = false;
                DataSetComponent.ActivityImageRow iinfo = new DataAccessComponent().RetrieveActivityImageInformation(ActivityID); ;
                int maxSizeKB = iinfo.FreeStorage;
                int fileSize = fileUpload1.PostedFile.ContentLength / 1024;

                DataSetComponent.ActivityImageDetailRow dr = GetData();
                ...
                if (System.Text.RegularExpressions.Regex.IsMatch(fileUpload1.PostedFile.ContentType, "image/\\S+") && (fileUpload1.PostedFile.ContentLength > 0))
                {
                    ...
                    {
                        int imageID = 0;
                        new BusinessFunctionComponent().CreateActivityImage(dr, out imageID, fileSize);
