...
namespace HealthyClub.Providers.Web.UserControls
{
    public partial class ProfileImageUC : System.Web.UI.UserControl
    {
        ...
        private void CheckUserImage()
        {
            ...
            var dr = new DataAccessComponent().RetrieveUserImageInformation(ProviderID);
            if (dr == null)
            {
        ...
        private void SetDataSource()
        {
            ...
            if (dr != null)
            {
                ...
                var imageDts = new DataAccessComponent().RetrieveUserImages(ProviderID);
                ListView1.DataSource = imageDts;
                ListView1.DataBind();
            }
        }

        private DataSetComponent.UserImageDetailRow GetData()
        {
            int imagesCount = new DataAccessComponent().RetrieveUserImagesCount(ProviderID);
        ...
        protected void imgBtnUpload_Click(object sender, EventArgs e)
        {
            ...
            var dt = new DataSetComponent.UserImageDetailDataTable();
                        ...
                        if (hfc.Count <= 10)    // 10 FILES RESTRICTION.
                        {
                            for (int i = 0; i <= hfc.Count - 1; i++)
                            {
                                ...
                                if (hpf.ContentLength > 0)
                                {
                                    ...
                                    {
                                        var dr = dt.NewUserImageDetailRow();
                                        dr.UserID = ProviderID;
            ...
            if (uploaded > 0)
            {
                ...
                new BusinessFunctionComponent().AddNewProviderImages(ProviderID, dt, usrImagDet);
                SetDataSource();
            }

        }

        private DataSetComponent.UserImageRow GetUserImageInfo()
        {
            var dr = new DataSetComponent.UserImageDataTable().NewUserImageRow();
