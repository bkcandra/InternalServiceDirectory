...
namespace HealthyClub.Providers.Web.UserControls
{
    public partial class ProviderAcoountSettingUC : System.Web.UI.UserControl
    {
        ...
        {
            var dr = new DataAccessComponent().RetrieveProviderProfiles(ProviderID);
            if (dr != null)
            {
        ...
        private bool authUser()
        {
            if (Context.User.Identity.IsAuthenticated)
            {
                ...
                if (ProviderID != Guid.Empty.ToString() && !string.IsNullOrEmpty(ProviderID))
                    return true;
                else
                {
                    Response.Redirect("~/Account/Login.aspx");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            if (!isError())
            {
                ...
                new DataAccessComponent().UpdateProviderProfiles(dr);
                if (chkBoxChangeActivityEmail.Checked)
                {
                    if (Email.Text.ToLower() != hdnEmailAddress.Value.ToLower())
                        new BusinessFunctionComponent().ChangeActivityEmailAddress(ProviderID, hdnEmailAddress.Value, Email.Text);
                }
                SetProviderInformation();
